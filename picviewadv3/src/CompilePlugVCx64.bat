...
@set fileversion_str=3.0 build 8 x64
@set MyDir=%CD%
@set companyname=Eugene Roshal ^& FAR Group
@set filedescription=PicView Advanced for Far Manager x64
...
@cl /Zp8 /O2 /W2 /Gy /GF /J /GS- /Gr /GR- /EHs-c- /LD %PlugName%.cpp /D_CRT_SECURE_NO_WARNINGS /D_CRT_NONSTDC_NO_WARNINGS /D_CRT_NON_CONFORMING_SWPRINTFS /DUNICODE /D_UNICODE /link /subsystem:console /machine:AMD64 /noentry /nodefaultlib /def:%PlugName%.def kernel32.lib advapi32.lib user32.lib MSVCRT.LIB libgfl64.lib gdi32.lib %PlugName%.res /map:"..\%PlugName%.map" /out:"..\%PlugName%.dll" /merge:.rdata=.text

copy /Y "%~dp0libgflx64\libgfl340.dll" "..\libgfl340.dll"
